...
IHubContext<NotificationHub> hubContext):BaseApiController
{
    ...
    public async Task<IActionResult> StripeWebhook()
    {
        ...
        {      
            ...
            return StatusCode(StatusCodes.Status500InternalServerError, "web hook error");
        }
        catch (Exception ex)
        {      
    ...
    private async Task HandlePaymentIntentSucceeded(PaymentIntent intent)
    {
        if(intent.Status == "succeeded")
        {
            ...
            var order = await unit.Repository<Core.Entities.OrderAggregates.Order>()
            .GetEntityWithSpec(spec) ?? throw new Exception("Order not found");
